...
<div class="popover-content">
    <?php if ($entity->voteUpCount == 1): ?>
        ...
        <?php else: ?>
            <?= @textf('LIB-AN-VOTE-ONE-VOTED', @name(@service('repos:actors.actor')->fetch(end($entity->voterUpIds->toArray())))) ?>
        <?php endif ?>
    <?php elseif ($entity->voteUpCount > 1): ?>
        <?php if ($entity->voterUpIds->offsetExists($viewer->id)): ?>
            ...
            <?php else: ?>
                <?= @textf('LIB-AN-VOTE-YOU-AND-OTHER-VOTED', $entity->voteUpCount - 1) ?>
            <?php endif ?>
        <?php else: ?>
            <?= @textf('LIB-AN-VOTE-OTHER-VOTED', $entity->voteUpCount) ?>
        <?php endif ?>
    <?php endif ?>
