 ...
 *      //if the person is following the actor, then actor is leading the person
    ...
    public function removeFollower($actor)
    {
        ...
        if (count($subscribers)) {
            $this->getService('repos://site/base.node')
                ->update(
                    "subscriber_ids = @remove_from_set(subscriber_ids,{$actor->id}), subscriber_count = @set_length(subscriber_ids)",
                    $subscribers
                );
