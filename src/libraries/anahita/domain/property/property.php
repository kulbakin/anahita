...
class AnDomainProperty extends KObject 
{
    ...
    static protected function _has(KConfig $config)
    {
        ...
        if (is_numeric($config->cardinality) && (int) $cardinality == 1) {
            ...
            if ( ! $property) {
                $property = $child_description->setRelationship($relationship->getChildKey(), array(
                    'type' => 'belongs_to',
            ...
            $through_one_to_many_target = $relationship->getTargetRepository()->getDescription()->setRelationship($parent, array(
                'cardinality'  => $config->cardinality,
                'type'         => 'has',
                'child_key'    => $relationship->getTargetChildKey(),
