...
class AnHelperArray extends KHelperArray
{
    /**
     * Index flags
     * (set string constants unlikely to be used as array indexes)
     */
    const FIRST_INDEX = '__AN_HELPER_ARRAY_FIRST_INDEX';
    const LAST_INDEX  = '__AN_HELPER_ARRAY_LAST_INDEX';
     ...
     * The index can be set as AnHelperArray::FIRST_INDEX or
     * AnHelperArray::LAST_INDEX to fetch first or last element
     * respectively
     * 
     * @param array $array
     * @param mixed $index
    ...
    public static function getValueAtIndex($array, $index)
    {
        ...
        if ($index == self::FIRST_INDEX and count($array)) {
            $value = reset($array);
        } elseif ($index == self::LAST_INDEX and count($array)) {
            $value = end($array);
            reset($array);
